// app/log/day/[date]/page.js — 그날 기록 상세 (수면 + 루틴 타임라인, 읽기 전용).
// 최근 기록 탭 → 여기로. 인증형(미들웨어 /log 보호).
  ...
  return (
    <AppShell>
      <div className="px-4 pt-[calc(env(safe-area-inset-top)+14px)] pb-2 flex items-center gap-3">
        <Link href="/log" className="w-9 h-9 -ml-1.5 rounded-lg flex items-center justify-center active:bg-muted"><ChevronLeft className="w-6 h-6" /></Link>
        <h1 className="text-[20px] font-bold tracking-[-0.3px]">{dateLabel}</h1>
      </div>
