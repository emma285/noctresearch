"use client";
// 코치 배정 폼 — 다음 세션·상태·프로그램 주차를 마스터 DB에 직접 저장. (/coach/clients/[email])
...
export default function CoachManageForm({ athlete }) {
  ...
  const [week, setWeek] = useState(athlete.week != null ? String(athlete.week) : "");
  ...
  async function save() {
    ...
    try {
      const r = await fetch("/api/coach/master", {
        ...
        body: JSON.stringify({ pageId: athlete.pageId, nextSession, status, week }),
      });
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4 mt-2">
      <div className="text-[13px] font-bold text-navy mb-3">다음 세션</div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-4">
        <div>
          <div className="text-[13px] font-bold text-navy mb-2">상태</div>
        ...
        <div>
          <div className="text-[13px] font-bold text-navy mb-2">프로그램(주)</div>
          <input type="number" inputMode="numeric" value={week} onChange={(e) => setWeek(e.target.value)} placeholder="예: 8" className={sel + " w-full"} />
        </div>
      </div>

      <button type="button" onClick={save} disabled={saving}
        className="w-full mt-4 bg-primary text-white text-[15px] font-semibold py-3 rounded-xl active:opacity-90 disabled:opacity-60">
