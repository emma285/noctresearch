...
// 이 선수에게 적용된 프로토콜 slug 목록 (sleep_targets.protocolEventId distinct).
export async function getAppliedProtocolSlugs(clientId) {
  if (!clientId) return [];
  try {
    const rows = await db.select({ p: sleepTargets.protocolEventId }).from(sleepTargets).where(eq(sleepTargets.clientId, clientId));
    return [...new Set(rows.map((r) => r.p).filter(Boolean).map(String))];
  } catch (e) {
    console.error("getAppliedProtocolSlugs failed:", e?.message);
    return [];
  }
}
