...
export async function updateMasterFields(pageId, fields = {}) {
  ...
  try {
    ...
    // '다음 세션' 날짜를 바꿨으면 다음 회차(최고 회차·미완료) 세션 행 session_at에도 반영 → 세션 목록에 뜸.
    if (fields.nextSession !== undefined) await syncNextUnfinishedSessionDate(pageId, fields.nextSession || null);
    return true;
  } catch (e) {
...
// 마스터 nextSession → 다음 회차(최고 회차이면서 아직 노트 없음/미공개) 세션 행 session_at 동기화.
// 최고 회차가 이미 끝난(공개/노트) 상태면 손대지 않음(과거 세션 날짜 덮어쓰기 방지).
async function syncNextUnfinishedSessionDate(clientId, iso) {
  try {
    const [top] = await db.select({ id: sessions.id, published: sessions.published, detail: sessions.detail })
      .from(sessions).where(eq(sessions.clientId, clientId)).orderBy(desc(sessions.n)).limit(1);
    if (!top) return;
    const done = top.published || !!(top.detail && top.detail.chief);
    if (done) return;
    await db.update(sessions).set({ sessionAt: iso ? new Date(iso) : null }).where(eq(sessions.id, top.id));
  } catch (e) { console.error("syncNextUnfinishedSessionDate failed:", e?.message); }
}

// 개별 회차 세션의 날짜(session_at)를 직접 설정/해제. sessionAt="" 이면 미정으로 지움.
// 이 세션이 선수의 '다음 세션'(최고 회차 & 미완료)이면 clients.nextSession도 동기화(포털·대시보드 일관성).
export async function updateSessionDate(id, sessionAt) {
  if (!id) return { ok: false, reason: "no-id" };
  const iso = sessionAt || "";
  const dt = iso ? new Date(iso) : null;
  if (iso && isNaN(dt)) return { ok: false, reason: "bad-date" };
  try {
    const cur = (await db.select().from(sessions).where(eq(sessions.id, id)).limit(1))[0];
    if (!cur) return { ok: false, reason: "no-session" };
    await db.update(sessions).set({ sessionAt: dt }).where(eq(sessions.id, id));
    // 최고 회차 & 미완료면 마스터 nextSession도 맞춰줌(선수 포털·대시보드가 이 값을 봄).
    const done = cur.published || !!(cur.detail && cur.detail.chief);
    if (!done && cur.clientId) {
      const [top] = await db.select({ n: sessions.n }).from(sessions)
        .where(eq(sessions.clientId, cur.clientId)).orderBy(desc(sessions.n)).limit(1);
      if (top && Number(top.n) === Number(cur.n)) {
        try { await db.update(clients).set({ nextSession: dt, updatedAt: new Date() }).where(eq(clients.id, cur.clientId)); }
        catch (e) { console.error("updateSessionDate master sync failed:", e?.message); }
      }
    }
    return { ok: true };
  } catch (e) { console.error("updateSessionDate failed:", e?.message); return { ok: false, reason: e?.message }; }
}
...
async function createNextSession(session) {
  const nextN = Math.floor(session.n || 0) + 1;   // 0.5 회차(예: 1.5) 다음은 다음 정수 회차(2)로
  ...
  // 날짜 우선순위: 노트 detail.nextSessionDate(전사서 캐치) → 없으면 코치가 우측에서 고른 clients.nextSession 폴백
  let iso = session.detail?.nextSessionDate || null;
  if (!iso) {
    try {
      const [c] = await db.select({ nextSession: clients.nextSession }).from(clients).where(eq(clients.id, session.clientId));
      if (c?.nextSession) iso = c.nextSession;
    } catch (e) { console.error("createNextSession client nextSession fallback failed:", e?.message); }
  }
