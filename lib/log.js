...
export async function getLogTimeline(email, endDate, numDays = 4) {
  ...
  try {
    ...
    for (const p of rows) {
      const d = p.data || {};
      if (p.kind === "sleep") sleeps.push({ date: p.date, bed: d.bed, sol: d.sol ?? null, wake: d.wake, out: d.outbed ?? null, feel: (d.feel || []).join(", "), memo: d.memo || "", woke: d.woke ?? 0, waso: d.waso ?? null });
      else routines.push({ date: p.date, t: parseTime(d.time), type: d.type || "etc", d: [d.detail, d.dur].filter(Boolean).join("·"), dur: durToMin(d.dur) });
    }
