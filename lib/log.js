...
export async function getLogTimeline(email, endDate, numDays = 4) {
  ...
  try {
    ...
    for (const p of rows) {
      ...
      else routines.push({ date: p.date, t: parseTime(d.time), type: d.type || "etc", d: [d.detail, d.dur].filter(Boolean).join("·"), text: d.text || d.memo || d.note || "", dur: durToMin(d.dur), tz: d.tz || null });
    }
...
// 선수의 가장 오래된 로그 날짜(YYYY-MM-DD). 없으면 null. 타임라인 전체 기간 산정용.
export async function getFirstLogDate(email) {
  if (!email) return null;
  try {
    const rows = await db.select({ date: logs.date }).from(logs)
      .where(eq(logs.userEmail, email)).orderBy(asc(logs.date)).limit(1);
    return rows[0]?.date || null;
  } catch (e) {
    console.error("getFirstLogDate failed:", e?.message);
    return null;
  }
}
